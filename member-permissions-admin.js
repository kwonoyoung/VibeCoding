(()=>{
  ...
  style.textContent=`
    ...
    .perm-controls{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.perm-controls select{padding:9px 11px;border:1px solid #cadbe4;border-radius:9px;background:#fff;min-width:300px}
    ...
    .usage-summary{padding:14px 20px;background:#f8fbfc;border-bottom:1px solid #e5edf1;display:flex;gap:10px;flex-wrap:wrap;align-items:center}.usage-chip{padding:7px 10px;border-radius:999px;background:#eef5f7;color:#385a69;font-size:12px;font-weight:850}.usage-chip strong{color:#0b6f67}
  ...
  card.innerHTML=`<div class="perm-head"><div><h2>회원별 메뉴 사용 권한 · 사용량</h2><p>회원별 메뉴 허용 여부와 실제 메뉴 사용횟수, 최초·최근 사용일시를 확인합니다.</p></div><div class="perm-controls"><select id="permMember"><option value="">회원을 선택하세요</option></select><button class="perm-btn all-on" id="permAllOn">전체 허용</button><button class="perm-btn all-off" id="permAllOff">전체 차단</button></div></div><div class="usage-summary" id="usageSummary"><span class="usage-chip">회원을 선택하면 사용량이 표시됩니다.</span></div><div class="perm-table"><table><thead><tr><th>메뉴명</th><th>페이지</th><th>사용 여부</th><th>상태</th><th>사용횟수</th><th>최초 사용</th><th>최근 사용</th></tr></thead><tbody id="permRows"><tr><td colspan="7" class="perm-msg">회원을 선택하면 메뉴 권한과 사용량이 표시됩니다.</td></tr></tbody></table></div>`;
  const main=document.querySelector('main.wrap')||document.body;main.appendChild(card);

  const memberSel=document.getElementById('permMember'),permRows=document.getElementById('permRows'),usageSummary=document.getElementById('usageSummary');
  ...
  async function render(){
    const uid=memberSel.value;if(!uid){permRows.innerHTML='<tr><td colspan="7" class="perm-msg">회원을 선택하면 메뉴 권한과 사용량이 표시됩니다.</td></tr>';usageSummary.innerHTML='<span class="usage-chip">회원을 선택하면 사용량이 표시됩니다.</span>';return;}
    ...
    usageSummary.innerHTML=`<span class="usage-chip">총 사용횟수 <strong>${total.toLocaleString()}회</strong></span><span class="usage-chip">사용한 메뉴 <strong>${usedMenus}개</strong></span><span class="usage-chip">최근 사용 <strong>${esc(fmt(last))}</strong></span>`;
